...
public:
    ...
    f64 game_milli = TARGET_MS_PER_FRAME;

    ImGuiIO* io = nullptr;
    f32 iot_display_scale = 1.5f;
...
namespace ui
{
    ...
    {        
        ...
        {
            auto game_ms = min((f32)(state.game_milli), 999.999f);
            auto frame_ms = min((f32)(state.frame_nano / MICRO), 999.999f);
            ImGui::Text("size = %u x %u | scale = %.1f | frame = %7.3f/%7.3f ms", width, height, state.game_window_scale, game_ms, frame_ms);
        }
    ...
    static bool texture_window(cstr label, ImTextureID image_texture, u32 width, u32 height, f32 scale = 1.0f)
    {
        auto open = ImGui::Begin(label);
    ...
    static void game_control_window(EngineState& state)
    {
        ...
        constexpr auto plot_max = 15.0f; // Magic!
        constexpr auto plot_size = ImVec2(0, 120.0f);
        constexpr auto data_stride = sizeof(f32);

        char overlay[32] = { 0 };
        stb::qsnprintf(overlay, 32, "frame time: %.2f / 15 ms", state.frame_times.current_avg());
